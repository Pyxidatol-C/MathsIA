...
Let $n$ be a nonnegative integer, and let $\Omega_n$ be the set of all self-avoiding paths of length $n$ departing from the origin. Corollary~\ref{cor:inf_cluster} tells us that
...
\begin{proof}
Using the reasoning from Figure~\ref{fig:nb_self_avoiding_path} above, we can deduce that $\abs{\Omega_n} \leq 4 \times 3^{n - 1}$.
\end{proof}
...
% \subsection{Yes Infinite Clusters}
