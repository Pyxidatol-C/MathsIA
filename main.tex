...
We call a percolation an \textit{outcome}; e.g., the leftmost panel of Figure~\ref{fig:3_realisations} shows an outcome when $p = 0.2$. An \textit{event} is a set of zero or more outcomes, and depends on finitely many edges. We say an event \textit{occurs} if the outcome belongs to the event.
\end{defn}
...
Using the definition from Example~\ref{ex:cluster_of_size_n}, for integers $m$ and $n$, if $n > m \geq 0$, we have $\{\abs{C} \geq n\} \subseteq \{\abs{C} \geq m\}$ and hence $\P(\abs{C} \geq n) \leq \P(\abs{C} \geq m)$. We then define
\[
\P(\abs{C} = \infty) 
...
To start with, we want to know for which values of $p$ there is no infinite cluster on the square lattice, i.e., $\theta(p) = 0$. We do this by finding a upper bound on $\theta(p)$.

\begin{prop}\label{prop:self_avoiding_path}
For positive integer $n$, we have $\abs{C} \geq n$ if and only if there is a \textit{self-avoiding path} of length $n$ of open edges from $O$.
\end{prop}
\begin{proof}
Recall that a self-avoiding path of length $n$ is a sequence $v_1, v_1, \dots, v_n$
\end{proof}

\begin{prop}% number path
