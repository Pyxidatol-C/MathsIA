...
\end{thm}
\begin{proof}
We begin by finding an upper bound for the probability that $\abs{C}$ is finite.
\begin{align}
    \P(\abs{C} < \infty)
...
\begin{align*}
    \theta(p) &= \P(\abs{C} = \infty) = 1 - \P(\abs{C} < \infty)\\
    \P(\abs{C} < \infty) &= 1 - \theta(p).
\end{align*}
...
\subsection{Critical Value}\label{subsec:pc}
...
Blah blah blah why we are interested in the cubic lattice

[Paths in the cubic lattice]

[Cubic lattice contains $\Z^2$]

[Blah blah]

\input{figures/z3_paths.tex}

\begin{quest}\label{q:pc_on_Z3}
